...
<tasks>
    <link href="self"
          href="${taskCollectionResource.uriInfo.absolutePath}?start=${taskCollectionResource.start}&limit=${taskCollectionResource.limit}"/>
    #if ( $response.size() == $taskCollectionResource.limit)
    #set( $next = $taskCollectionResource.start + $taskCollectionResource.limit )
    <link href="next"
          href="${taskCollectionResource.uriInfo.absolutePath}?start=${next}&limit=${taskCollectionResource.limit}"/>
    #end
    ...
    <link href="previous"
          href="${taskCollectionResource.uriInfo.absolutePath}?start=${previous}&limit=${taskCollectionResource.limit}"/>
    #end
